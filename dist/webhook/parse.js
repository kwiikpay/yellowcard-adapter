 ...
 */
export function parseYcWebhookPayload(raw) {
    const payload = (raw ?? {});
    const event = String(payload.event ?? payload.type ?? payload.eventType ?? "");
    const dataObj = (payload.data ?? payload);
    const ycOrderId = String(dataObj.id ?? dataObj.collectionId ?? dataObj.paymentId ?? "");
 ...
 */
export function classifyKindFromSequenceId(sequenceId) {
    if (!sequenceId)
        return "unknown";
    if (/^[a-z0-9]+-in-/i.test(sequenceId))
        return "inbound";
    if (/^[a-z0-9]+-/i.test(sequenceId))
        return "outbound";
    return "unknown";
}
