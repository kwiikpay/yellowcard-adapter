 ...
 */
export function normalizeKycFromUserMetadata(meta, email) {
    if (!meta)
        return { email };
    const pick = (k) => {
        const v = meta[k];
 ...
 */
export function deriveCustomerName(kyc, fallback = "Kwiikpay Customer") {
    ...
    if (composed.length > 0)
        return composed;
    if (kyc.email && kyc.email.trim().length > 0)
        return kyc.email.trim();
    return fallback;
}
